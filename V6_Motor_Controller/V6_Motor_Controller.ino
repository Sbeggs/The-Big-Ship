//--------------------------------------------------------------------------------
// 4 MOTOR RUNNING CODE - VERSION 6 - TUNED
//--------------------------------------------------------------------------------
...
//             --> implements a basic realign
//             --> has issues with true true
...
unsigned int shiftpwm_A1 = 220;
unsigned int shiftpwm_B1 = 220;
unsigned int shiftpwm_A2 = 220;
unsigned int shiftpwm_B2 = 220;
...
void runA2(int speed, boolean rev) {
  ...
  digitalWrite(A3, LOW);   //Disengage the Brake for Channel A
  analogWrite(PWMpinA2, speed);   //Spins the motor on Channel A at speed
}
...
void runB2(int speed, boolean rev) {
  ...
  analogWrite(PWMpinB2, speed);   //Spins the motor on Channel B at speed
}
...
void shiftLeft() {
  ...
  runA2(shiftpwm_A1, false);
  runB2(shiftpwm_B2, true);
...
void shiftRight() {
  ...
  runA2(shiftpwm_A2, true);
  runB2(shiftpwm_B2, false);
...
void shiftForward() {
  ...
  runA1(shiftpwm_A1, true);
  runB1(shiftpwm_B1, false);
...
void shiftBackward() {
  ...
  runA1(shiftpwm_A1, false);
  runB1(shiftpwm_B1, true);
